import tqdm
from openprompt.data_utils.text_classification_dataset import  SymptomProcessor,WomanProcessor
import torch
...
#the dataset named Symptom
if args.dataset == "symptom":
    dataset['train'] = SymptomProcessor().get_train_examples("datasets/TextClassification/symptom/")
    dataset['test'] = SymptomProcessor().get_test_examples("datasets/TextClassification/symptom/")
    class_labels =SymptomProcessor().get_labels()
    scriptsbase = "TextClassification/symptom"
    scriptformat = "txt"
    cutoff=0.5
    max_seq_l = 128
    batch_s = args.batch_size
#the dataset named Gynecology
elif args.dataset == "woman":
    dataset['train'] = WomanProcessor().get_train_examples("datasets/TextClassification/woman/")
    dataset['test'] = WomanProcessor().get_test_examples("datasets/TextClassification/woman/")
    class_labels =WomanProcessor().get_labels()
    scriptsbase = "TextClassification/woman"
    scriptformat = "txt"
    cutoff=0.5
    max_seq_l = 128
    batch_s = args.batch_size
